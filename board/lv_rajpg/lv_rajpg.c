...
enum io_source_type {
    RAJPEG_IO_SOURCE_C_ARRAY,
    RAJPEG_IO_SOURCE_DISK,
};
...
typedef struct {
    ...
    int img_cache_x_res;
    int img_cache_y_res;
    uint8_t * raw_rajpg_data;              //Used when type==SJPEG_IO_SOURCE_C_ARRAY.
    uint32_t raw_rajpg_data_size;          //Num bytes pointed to by raw_sjpg_data.
    uint32_t raw_rajpg_data_next_read_pos; //Used for all types.
} io_source_t;


typedef struct {
    uint8_t * rajpeg_data;
    uint32_t rajpeg_data_size;
    int rajpeg_x_res;
    int rajpeg_y_res;
    int rajpeg_total_frames;
    int rajpeg_single_frame_height;
    int rajpeg_cache_frame_index;
...
typedef enum {
    ...
    JPEG_ERR_NO_SOI = -1,
    JPEG_ERR_NO_SOF = -2,
    JPEG_ERR_INVALID_MARKER = -3,
    JPEG_ERR_BUFFER_END = -4
} JPEG_Error_t;

// Marker types we're interested in
#define JPEG_MARKER_SOI  0xD8    // Start of Image
#define JPEG_MARKER_SOF0 0xC0    // Baseline DCT
#define JPEG_MARKER_APP0 0xE0    // JFIF APP0
#define JPEG_MARKER_EOI  0xD9    // End of Image

// Helper function for reading 16-bit big-endian values
// Cortex-M4 is little-endian
__attribute__((always_inline)) static inline uint16_t read_be16(const uint8_t *ptr) {
    return ((uint16_t)ptr[0] << 8) | ptr[1];
}

// Fast marker search optimized for Cortex-M4
__attribute__((always_inline)) static inline uint8_t find_next_marker(const uint8_t **ptr, const uint8_t *end) {
    while (*ptr < end) {
        if (**ptr == 0xFF) {
            (*ptr)++;
            if (*ptr < end && **ptr != 0xFF) {
...
JPEG_Error_t JPEG_ParseHeader(const uint8_t *buffer, uint32_t size, JPEG_Info_t *info) {
    ...
    // Check minimum size and SOI marker
    if (size < 4 || ptr[0] != 0xFF || ptr[1] != JPEG_MARKER_SOI) {
        return JPEG_ERR_NO_SOI;
    }
    ptr += 2;
    ...
    while (ptr < end - 1) {
        // Find next marker
        marker = find_next_marker(&ptr, end);
        if (!marker) {
            break;
        }

        // Check if we have enough bytes to read segment size
        if (ptr + 2 > end) {
            return JPEG_ERR_BUFFER_END;
        }

        // Read segment size (includes size bytes but not marker)
        segment_size = read_be16(ptr);
        if (segment_size < 2 || ptr + segment_size > end) {
            return JPEG_ERR_BUFFER_END;
        }

        ptr += 2; // Skip size field

        // Process markers we're interested in
        switch (marker) {
            case JPEG_MARKER_APP0:
                if (segment_size >= 7) { // Minimum size for JFIF header
                    // Check for JFIF signature
                    if (ptr[0] == 'J' && ptr[1] == 'F' && ptr[2] == 'I' &&
                        ptr[3] == 'F' && ptr[4] == 0) {
                        info->is_jfif = true;
                        info->version_major = ptr[5];
                        info->version_minor = ptr[6];
                    }
                }
                break;

            case JPEG_MARKER_SOF0:
                if (segment_size >= 7) { // Minimum size for SOF header
                    info->precision = ptr[0];
                    info->height = read_be16(ptr + 1);
                    info->width = read_be16(ptr + 3);
                    info->components = ptr[5];
                    info->is_valid = true;
                }
                break;
        ...
        ptr += segment_size - 2; // Move to next segment
    }

parse_complete:
    return info->is_valid ? JPEG_OK : JPEG_ERR_NO_SOF;
}

// Example function to process JPEG data in chunks
...
void JPEG_Init_Parser(JPEG_Parser_State_t *state) {
    state->bytes_processed = 0;
    state->state = 0;
    state->info.is_valid = false;
}

// Function to check if buffer contains specific pattern at offset
__attribute__((always_inline)) static inline bool check_pattern(const uint8_t *buf, uint32_t offset, const uint8_t *pattern, uint32_t len) {
    for (uint32_t i = 0; i < len && (offset + i) < CHUNK_SIZE; i++) {
...
// Example usage
void JPEG_Process_Example(void) {
    // Static buffer for reading chunks (stack friendly)
    static uint8_t buffer[CHUNK_SIZE];
    JPEG_Info_t info;

    // Read from your data source (e.g., SPI Flash, SD card)
    // This is just an example - implement actual reading logic
    uint32_t bytes_read = read_data_source(buffer, CHUNK_SIZE);

    // Parse header
    JPEG_Error_t result = JPEG_ParseHeader(buffer, bytes_read, &info);

    // Use the information
    if (result == JPEG_OK) {
        // Access info structure members
        uint16_t width = info.width;
        uint16_t height = info.height;

        // Example: Calculate required buffer size for MCU processing
        uint32_t mcu_size = info.components * 64; // 64 coefficients per component
